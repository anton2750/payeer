...
Также в теле запроса обязательным параметром является <code>ts</code>, значение которого равно текущему timestamp <b>в
миллисекундах</b>.<br>
<br>
Запрос будет обработан только если дойдет до нашего сервера в течение 60 секунд, чтобы исключить существенные задержки
сети.<br>
<br>
Максимальная разница параметра <code>ts</code> от реального времени не должна превышать 1 секунду, не забудьте настроить
синхронизацию времени на своем сервере.<br>
<br>
